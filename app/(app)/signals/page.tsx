...
import type { SignalRow } from '@/components/signals/TweetTimelineCard'
import { getKbSignals } from '@/lib/queries'

// 信号日志 = 带推特 call 时间线的透明信号 feed(列表 → 点开详情)。数据来自 KB(kb_signals.narrative_twitter)。
export default async function SignalsPage() {
  const { data } = await getKbSignals()
  const rows = (data || []) as unknown as SignalRow[]
  const list = rows.filter((r) => r.narrative_twitter?.status === 'generated')
  const now = Date.now()
    ...
    <>
      ...
      <main style={{ padding: '18px 22px', maxWidth: 760, margin: '0 auto' }}>
        <p style={{ fontSize: 12.5, color: 'var(--text-3)', margin: '0 0 16px', lineHeight: 1.6 }}>
          链上信号 + 推特 call 时间线,带时间戳透明落档。点任一信号看完整时间线 + 跳原推核验。聚合自公开信息,非投资建议。
        </p>
        ...
        {list.length === 0 ? (
          ...
          list.map((r) => <SignalListItem key={r.ca} row={r} now={now} />)
        )}
      </main>
    </>
  )
