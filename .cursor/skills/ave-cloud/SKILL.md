...
python scripts/ave_data_rest.py <command> [options]
```
...
The script is at `scripts/ave_data_rest.py` relative to the skill root.
...
Full list of ~90 values is in `scripts/ave_data_rest.py` (`VALID_PLATFORMS`). Use `--help` to list.
...
```bash
python scripts/ave_data_rest.py kline-token --address <token> --chain <chain> \
  [--interval <minutes>] [--size <count>]

python scripts/ave_data_rest.py kline-pair --address <pair> --chain <chain> \
  [--interval <minutes>] [--size <count>]
```
...
```bash
API_PLAN=pro AVE_API_KEY="your_key" python scripts/ave_data_rest.py wss-repl
```
...
```bash
python scripts/ave_data_rest.py watch-tx --address <pair_address> --chain <chain> [--topic tx]
```
...
```bash
python scripts/ave_data_rest.py watch-kline --address <pair_address> --chain <chain> [--interval k60]
```
...
```bash
python scripts/ave_data_rest.py watch-price --tokens <addr1>-<chain1> [<addr2>-<chain2> ...]
```
