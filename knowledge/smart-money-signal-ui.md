...
> 榜单页聪明钱信号 Tab 及信号卡片 UI。现有 Binance Skill（金黄风格）。数据统一来自 Binance Web3 API。
...
- **Tab**：「Binance Skill」与「Solana Pump 榜单」「zhizhilabs 精选」同属 `.tabs`，使用相同 `.tab-btn` 样式。
- **风格**：Binance 金黄 `--bn-yellow`。
