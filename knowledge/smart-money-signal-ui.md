...
> 榜单页聪明钱信号 Tab 及信号卡片 UI。数据统一来自 Binance Web3 API。另有「binance聪明钱流入排行」Tab，以表格形式按净流入金额排序，支持每 60 秒自动刷新。
...
- **Tab**：「binance聪明钱信号」「binance聪明钱流入排行」与「Solana Pump 榜单」「zhilabs 精选」同属 `.tabs`，使用相同 `.tab-btn` 样式。
- **聪明钱信号**：卡片风格，金黄 `--bn-yellow` 主色。
- **聪明钱流入排行**：表格形式，#、代币、符号、链、净流入、市值、聪明钱数；按净流入金额排序；每 60 秒自动刷新。
